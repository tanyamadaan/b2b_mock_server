...
export const initiateConfirmController = async (req: Request, res: Response) => {
  const { scenario, transactionId } = req.body;

  const transactionKeys = await redis.keys(`${transactionId}-*`);
  const ifTransactionExist = transactionKeys.filter((e) => e.includes('on_init-to-server'))
...
const intializeRequest = async (req: Request, res: Response, transaction: any, scenario: string) => {
  const {
    context,
    message: {
      order: { provider, locations, payments, fulfillments, xinput, items, ...order },
    },
  } = transaction
  ...
  const customized = checkIfCustomized(order.items)
  ...
  const confirm = {
    ...
    order: {
      ...
      quote: (customized) ? quoteCreatorServiceCustomized(order.items) : quoteCreatorService(order.items),
      payments: [{
        ...payments[0],
        status: "PAID"
      }],
