...
const statusRequest = async (req: Request, res: Response, transaction: any, scenario: any) => {
	...
	const responseMessage: any = {
		order: {
			...
			payments: transaction.message.order.payments.map((payment: any) => ({
				...
				"@ondc/org/settlement_details": payment["@ondc/org/settlement_details"].map((itm: any) =>
				({
					...itm,
	...
	switch (scenario) {
		case "delivered":
			...
			responseMessage.order.fulfillments.forEach((itm: any) => itm.state.descriptor.code = "Order-delivered")
			break;
		...
		case "picked-up":
			responseMessage.order.state = "In-progress"
			responseMessage.order.fulfillments.forEach((itm: any) => itm.state.descriptor.code = "Order-picked-up")
			break;
		case "proforma-invoice":
			...
			responseMessage.order.payments.forEach((itm: any) => itm["@ondc/org/settlement_details"].forEach((itm: any) => {
		...
		case "bpp-payment-error":
			...
			responseMessage.order.payments.forEach((itm: any) => itm.status = "NOT-PAID")
			responseMessage.order.fulfillments.forEach((itm: any) => itm.state.descriptor.code = "Order-delivered")
			responseMessage.order.payments.forEach((itm: any) => itm["@ondc/org/settlement_details"].forEach((itm: any) => {
		...
		case "self-picked-up":
			responseMessage.order.fulfillments.forEach((itm: any) => itm.state.descriptor.code = "order-picked-up")
			responseMessage.order.fulfillments.forEach((itm: any) => itm.type = "Self-Pickup")
			break;
		default:
			responseMessage.order.documents = [
				{
