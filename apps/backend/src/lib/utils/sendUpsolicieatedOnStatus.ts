import { AGRI_HEALTHCARE_STATUS } from "./apiConstants";
import { Fulfillment, Stop } from "./interfaces";
import { findIncompleteOnConfirmCalls, redisFetchFromServer } from "./redisFetch";
  ...
  try {
    ...
    if (data && data.length > 0) {
      data.map(async (ele: any) => {
        ...
        const on_status = await redisFetchFromServer("on_status", ele?.request?.context?.transaction_id);
        if (on_status) {
        ...
        if (lastStatus !== AGRI_HEALTHCARE_STATUS[6]) {
          ...
          await sendStatusAxiosCall(
            context,
            responseMessage,
            `${context.bap_uri}${context.bap_uri.endsWith("/") ? "on_status" : "/on_status"}`,
            `on_status`,
            "healthcare-service"
          );
        }
